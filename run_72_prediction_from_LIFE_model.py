...
for training in ['training_life_only', 'training_2samp']:
    wd_root_path = os.path.join(PATH', training)
    ...
    for life_subjects_selection_crit in life_subjects_selection_crit_list:
        ds_dir = os.path.join(ds_root_path, life_subjects_selection_crit)
        trained_model_template = {
            'trained_model': 'group_learning_prepare_data/{ana_stream}trained_model/' +
                             '_multimodal_in_data_name_{multimodal_in_data_name}/_selection_criterium_' + life_subjects_selection_crit + '/' +
