import os
from LeiCA_LIFE.learning.learning_stacking_utils import stacking

date_str = '20160504'

target = 'age'

selection_crit_nki = 'adult'

life_subjects_selection_crit_list = ['bothSexes_neuH_FD06']
in_data_path = PATH

for training in ['training_life_only', 'training_2samp']:
    root_path_template = os.path.join(in_data_path,
                                      'learning_out_from_LIFE_models_' + date_str + '/' + training + '/{life_subjects_selection_crit}/pdfs/single_source_model_reg_{reg}_predicted')
    out_path_template = os.path.join(in_data_path,
                                     'learning_out_from_LIFE_models_' + date_str + '/' + training + '/{life_subjects_selection_crit}/stacking/stacking_out_reg_{reg}')
